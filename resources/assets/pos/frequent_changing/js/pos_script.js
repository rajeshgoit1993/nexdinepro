...
$(document).ready(function () {
  ...
  $(document).on("click", ".remove_table_order", function () {
    ...
    swal(
      ...
      function () {
        this_action.remove();
        $.ajax({
          url: base_url + "/remove_a_table_booking_ajax",
          method: "POST",
          data: {
      ...
      swal(
        ...
        function () {
          ...
          if (notifications != "") {
            ...
            $.ajax({
              url: base_url + "/remove_multiple_notification_ajax",
              method: "POST",
              data: {
  ...
  $(document).on("click", ".single_serve_b", function () {
    ...
    $.ajax({
      url: base_url + "/remove_notication_ajax",
      method: "POST",
      data: {
  ...
  $(document).on("click", ".single_hold_sale", function () {
    ...
    $.ajax({
      url: base_url + "/get_single_hold_info_by_ajax",
      method: "POST",
      data: {
        hold_id: hold_id,
        csrf_irestoraplus: csrf_value_,
  ...
  $(document).on("click", ".single_last_ten_sale", function () {
    ...
    $.ajax({
      url: base_url + "/get_all_information_of_a_sale_ajax",
      method: "POST",
      data: {
        sale_id: sale_id,
        csrf_irestoraplus: csrf_value_,
      ...
      success: function (response) {
        ...
        $("#recent_sale_modal_details_vat").html(Number(response.gst).toFixed(ir_precision));
      },
      error: function () {
        alert(a_error);
      },
    ...
    if ($(".detail_hold_sale_holder .single_hold_sale").length > 0) {
      swal(
        ...
        function () {
          ...
          $.ajax({
            url: base_url + "/delete_all_holds_with_information_by_ajax",
            method: "POST",
            data: {
              csrf_irestoraplus: csrf_value_,
            },
    ...
    if ($(".single_hold_sale[data-selected=selected]").length > 0) {
      ...
      swal(
        ...
        function () {
          //delete all information of hold based on hold_id
          $.ajax({
            url: base_url + "/delete_all_information_of_hold_by_ajax",
            method: "POST",
            data: {
              hold_id: hold_id,
              csrf_irestoraplus: csrf_value_,
    ...
    if ($(".single_last_ten_sale[data-selected=selected]").length > 0) {
      ...
      if (role != "Admin") {
        ...
        swal(
          ...
          function () {
            $("#delete_sale_id").val("").val(sale_id)
             $("#delete_order_by_admin").addClass("active")
  ...
  $(document).on("click","#delete_order_cancel_button",function(e){
    e.preventDefault()
    $("#delete_order_by_admin").removeClass("active")
  })

   $(document).on("click", "#delete_order_button", function (e) {
    let sale_id = $("#delete_sale_id").val();
    let delete_remarks=$("#delete_remarks").val();
   
    if (delete_remarks != '') {

      $.ajax({
              url: base_url + "/cancel_particular_order_ajax",
              method: "POST",
              data: {
                sale_id: sale_id,
  ...
  $(document).on("click", "#create_bill_and_close", function (e) {
    ...
    if (
      ...
      if (print_type_bill == "web_browser") {
        ...
        $(".pos__modal__overlay").fadeOut(300);
        $.ajax({
          url: base_url + "/printSaleBillByAjax",
          method: "get",
          dataType: "json",
          data: {
            sale_id: sale_id,
      ...
      if (foundItems.hasOwnProperty(key)) {
        if (status == "veg" && foundItems[key].veg_item_status == "yes") {
          searched_category_items_to_show +=
            '<div class="single_item animate__animated animate__flipInX vg_'+foundItems[key].veg_item+' nvg_'+foundItems[key].non_veg_item+' beverage_'+foundItems[key].beverage_item+'"  id="item_' +
          ...
          status == "nonveg" &&
          foundItems[key].non_veg_item_status == "yes"
        ) {
          searched_category_items_to_show +=
            '<div class="single_item animate__animated animate__flipInX vg_'+foundItems[key].veg_item+' nvg_'+foundItems[key].non_veg_item+' beverage_'+foundItems[key].beverage_item+'"  id="item_' +
            foundItems[key].item_id +
  ...
  $(document).on("click", ".single_item", function () {
    ...
    let qty_current = 1;
     ...
     if(!check_current_qty(item_id,qty_current) && has_kitchen=="No"){
         ...
         let item_total_price = parseFloat(item_price*qty_current).toFixed(ir_precision);

         //get item/menu quantity from modal
         let item_quantity = qty_current;
         ...
         else
         {
          ...
          $("#item_total_price_table_"+item_id).html('').html(item_total_price)

          $("#item_quantity_table_"+item_id).html('').html(item_quantity)
  ...
  $(document).on("click", "#cancel_order_button", function (e) {
      ...
      swal(
        ...
        function () {
          let sale_id = $(
            ...
            $("#cancel_sale_id").val("").val(sale_id)
            $("#cancel_order_by_admin").addClass("active")
   ...
   $(document).on("click","#cancel_order_cancel_button",function(e){
    e.preventDefault()
    $("#cancel_order_by_admin").removeClass("active")
  })
...
$(document).on("click", "#cancels_order_button", function (e) {
    let sale_id = $("#cancel_sale_id").val();
    let delete_remarks=$("#cancel_remarks").val();
  ...
  $(document).on("click", "#table_button,#dine_in_button", function (e) {
    ...
    $(".bottom_person").val("1");
    $.ajax({
      url: base_url + "/get_all_tables_with_new_status_ajax",
      method: "GET",
      success: function (response) {
        response = JSON.parse(response);
        ...
        $(".table_image").attr("src", base_url + "/public/uploads/pos/table_icon2.png");
        for (let key in table_availability) {
          ...
          if (booked_sit != 0) {
            $("#single_table_info_holder_" + table_id)
              .find(".table_image")
              .attr("src", base_url + "/public/uploads/table_icon2_blue.png");
          }
        }
      },
      error: function () {
  ...
  $(document).on("click", "#last_ten_sales_button", function (e) {
    ...
    $(".pos__modal__overlay").fadeIn(200);
    $.ajax({
      url: base_url + "/get_last_10_sales_ajax",
      method: "GET",
      success: function (response) {
  ...
  let placeOrderSound = new Howl({
    src: [base_url + "/resources/assets/pos/assets/media/alert_alarm.mp3"],
  });
  $(document).on("click", "#place_order_operation", function (e) {
    ...
    if (customer_id == '' || !customer_id) {
      $("#select2-walk_in_customer-container").css("border", "2px solid red");
      setTimeout(function () {
        $("#select2-walk_in_customer-container").css("border", "none");
      }, 2000);
  ...
  $(document).on("click", "#finalize_order_button", function (e) {
    ...
    if (status == true) {
      ...
      if (sale_id > 0) {
        ...
        $("#last_invoice_id").val('').val(sale_id);
  ...
  $(document).on("click", "#add_customer", function (e) {
    ...
    let this_action = $(this);
    $.ajax({
      url: base_url + "/add_customer_by_ajax",
      method: "post",
      data: {
      ...
      success: function (response) {
        ...
        if (response > 0) {
          ...
          $.ajax({
            url: base_url + "/get_all_customers_for_this_user",
            method: "GET",
            success: function (response) {
              response = JSON.parse(response);
...
function show_all_items() {
  ...
  setTimeout(function () {
    ...
    for (let key in foundItems) {
      if (foundItems.hasOwnProperty(key)) {
        ...
        searched_category_items_to_show +=
          '<div class="single_item animate__animated all_item_custom vg_'+foundItems[key].veg_item+' nvg_'+foundItems[key].non_veg_item+' beverage_'+foundItems[key].beverage_item+'" data-veg_status="' +
          veg_status +
  ...
  $(document).on("click", "#hold_sale", function (e) {
    if ($(".order_holder .single_order").length > 0) {
      $.ajax({
        url: base_url + "/get_new_hold_number_ajax",
        method: "GET",
        success: function (response) {
...
function adjust_middle_side_cart_list() {
  ...
  let top_scroller = $(".top_scroller").height();
...
$(document).on("click", "#print_kot_modal", function (e) {
  ...
  order_info += '"tbl_name":"' + table_name + '",';
...
console.log(order_info)
  $.ajax({
    url: base_url + "/add_temp_kot_ajax",
    method: "post",
    data: {
      order: order_object,
      csrf_irestoraplus: csrf_value_,
    ...
    success: function (response) {
      if (response > 0) {
        ...
        if (print_type_kot == "web_browser") {
          let newWindow = open(
            base_url + "/print_temp_kot/" + Number(response),
            "Print KOT",
            "width=450,height=550"
          );
          newWindow.focus();
...
function do_addition_of_item_and_modifiers_price() {
  ...
  // alert(total_of_all_items_and_modifiers)
  //get total items in cart
  let total_items_in_cart = $(".order_holder .single_order").length;
...
function set_all_visible_discounted_item_price() {
  $(".single_order .first_portion .fifth_column span").each(function (i, obj) {
    ...
    let item_discount_value = $(this)
      ...
      .parent()
      .parent()
      .find(".third_column .qty_item_custom")
      .html();
    ...
    let actual_discount_amount = get_particular_item_discount_amount(
      ...
      if (item_discount_value.length > 0 && item_discount_value.substr(item_discount_value.length - 1) == "%") {
     var dis=(
      (parseFloat(this_item_original_price) * parseFloat(item_discount_value)*parseFloat(item_qty)) /
      parseFloat(100)
    ).toFixed(ir_precision);
     } else {
    var dis= parseFloat(item_discount_value).toFixed(ir_precision);
     }
    ...
    //   .html(discounted_item_price);
     ...
     $(this)
      ...
      .parent()
      .find(".fifth_column span")
      .html(parseFloat(this_item_original_price*item_qty) - parseFloat(dis));
...
function get_total_of_all_items_and_modifiers() {
  ...
  $(".single_order .first_portion").each(function (i, obj) {
   //
    let item_discount_value = $(this).children(".forth_column").children(".discount_cart_input").val();
   let this_item_original_price = parseFloat($(this).children(".second_column").children("span").html()).toFixed(ir_precision);
  item_discount_value = item_discount_value != "" ? item_discount_value : 0;
   
      if (item_discount_value.length > 0 && item_discount_value.substr(item_discount_value.length - 1) == "%") {
     var dis=(
      (parseFloat(this_item_original_price) * parseFloat(item_discount_value)*parseFloat($(this).children(".third_column").children("span").html())) /
      parseFloat(100)
    ).toFixed(ir_precision);
     } else {
    var dis= parseFloat(item_discount_value).toFixed(ir_precision);
    ...
    all_item_total_price = (
      parseFloat(all_item_total_price)-parseFloat(dis) + parseFloat($(this).children(".second_column").children("span").html()*$(this).children(".third_column").children("span").html())
    ).toFixed(ir_precision);
  ...
  $(".single_order .second_portion").each(function (i, obj) {
    all_modifiers_total_price = (
      parseFloat(all_modifiers_total_price) + parseFloat($(this).html()*$(this).children(".third_column").children("span").html())
    ).toFixed(ir_precision);
      ...
      item_total_price=parseFloat(item_total_price)-
              parseFloat(
                parseFloat(item_total_price) *
                  parseFloat(tmt_value_sub_total[0])
              ) / parseFloat(100)
...
function set_new_orders_to_view(sale_id) {
  $.ajax({
    url: base_url + "/get_new_orders_ajax",
    method: "GET",
    success: function (response) {
      response = JSON.parse(response);
      let order_list_left = "";
...
function get_all_hold_sales() {
  $.ajax({
    url: base_url + "/get_all_holds_ajax",
    method: "GET",
    success: function (response) {
      let orders = JSON.parse(response);
...
function update_order_status_to_invoiced(
  ...
  $.ajax({
    url: base_url + "/update_order_status_ajax",
    method: "POST",
    data: {
      sale_id: sale_id,
      close_order: false,
...
function print_invoice(sale_id) {
  ...
  if (Number(print_type) == 1) {
    if (print_type_invoice == "web_browser") {
      newWindow = open(
        base_url + "/print_invoice/" + Number(sale_id),
        "Print Invoice",
        "width=450,height=550"
      );
    } else {
...
function new_notification_interval() {
  $.ajax({
    url: base_url + "/get_new_notifications_ajax",
    method: "get",
    data: {
      a: 'NA',
    },
    success: function (response) {
     ...
     // console.log(notification_counter_update)
...
function get_customer_for_edit(customer_id) {
  $.ajax({
    url: base_url + "/get_customer_ajax",
    method: "POST",
    data: {
      customer_id: customer_id,
      csrf_irestoraplus: csrf_value_,
...
let productSound = new Howl({
  src: [base_url + "/resources/assets/pos/assets/media/access.mp3"],
});
let productSound2 = new Howl({
  src: [base_url + "/resources/assets/pos/assets/media/click.mp3"],
});
...
$(document).on("click", "#last_ten_feature_button", function (e) {
  ...
  $(".pos__modal__overlay").fadeIn(200);
  $.ajax({
    url: base_url + "/get_last_10_future_sales_ajax",
    method: "GET",
    success: function (response) {
      let orders = JSON.parse(response);
      let last_10_orders = "";
