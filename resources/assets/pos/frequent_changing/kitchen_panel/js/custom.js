  ...
  $(document).on("click", ".start_cooking_button", function () {
    ...
    if ($("#single_order_" + sale_id).attr("data-order-type") == "Dine In") {
      if (
        ...
        if (previous_id != "") {
          let url = base_url + "/update_cooking_status_ajax";
          $.ajax({
            url: url,
            method: "POST",
            data: {
      ...
      if (previous_id != "") {
        let url =
          base_url + "/update_cooking_status_delivery_take_away_ajax";
        $.ajax({
          url: url,
          method: "POST",
          data: {
  ...
  $(document).on("click", "#start_cooking", function () {
    if (
      ...
      if (
        ...
        if (previous_id != "") {
          $.ajax({
            url: base_url + "/update_cooking_status_ajax",
            method: "POST",
            data: {
              previous_id: previous_id,
              cooking_status: "Started Cooking",
...
let bell_new_order = new Howl({
  src: [base_url + "/resources/assets/pos/assets/media/kitchen_bell.mp3"],
});

refresh_orders();
function refresh_orders() {
  let url = base_url + "/get_new_orders_ajax";
  $("#refresh_it_or_not").html("Yes");
  $.ajax({
    url: url,
    method: "POST",
