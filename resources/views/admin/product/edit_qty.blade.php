...
{!! Form::open(["files"=>true,'id'=>'update_product_qty'])!!}
...
<?php 
$check_today_data=DB::table('physical_entries')->where([['outlet_id',$data->outlet_id],['ingredient_id',$data->item_code]])->whereDate('date',date('Y-m-d'))->first();
$consuption_sum=DB::table('franchise_sale_consuptions_of_menus')
         ->where([['outlet_id',(int)$data->outlet_id],['ingredient_id',(int)$data->item_code],['sync_status',0]])->sum('consumption');
          
    $waste_sum=DB::table('waste_ingredients')
         
           ->where([['outlet_id',(int)$data->outlet_id],['ingredient_id',(int)$data->item_code],['sync_status',0]])->sum('waste_amount');
