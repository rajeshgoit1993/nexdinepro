...
<div class="card direct-chat direct-chat-primary">
  ...
  ?>
   @if($request_segments=='Newly-Assigned-Factory-Order') 
 <input type="hidden" name="order" id="order" value="1">
    @elseif($request_segments=='Replied-to-Accounts-Factory-Order')
 <input type="hidden" name="order" id="order" value="2">
     @elseif($request_segments=='Ongoing-Factory-Order-Accounts')
 <input type="hidden" name="order" id="order" value="3">
  @elseif($request_segments=='Dispatched-Factory-Order')
 <input type="hidden" name="order" id="order" value="4">
 @elseif($request_segments=='Completed-Factory-Order-Accounts')
...
<script type="text/javascript">
 ...
 var APP_URL=$("#APP_URL").val();
  $.ajax({
        url:APP_URL+'/update_factory_delivered_status_by_accounts',
        data:form_data,
        type:'post',
        contentType: false,
        processData: false,
...
//
 ...
 var APP_URL=$("#APP_URL").val();
  $.ajax({
        url:APP_URL+'/update_factory_accept_status_by_accounts',
        data:form_data,
        type:'post',
        contentType: false,
        processData: false,
...
$(document).on("click",".accounts_enter_collection",function(){
   ...
   var APP_URL=$("#APP_URL").val();
     ...
     $.ajax({
        url:APP_URL+'/accounts_view_and_verify_collection_details_factory',
        data:{id:id},
        type:'post',
        // contentType: false,
        // processData: false,
 ...
 var APP_URL=$("#APP_URL").val();
  $.ajax({
        url:APP_URL+'/update_factory_order_status_account',
        data:form_data,
        type:'post',
        contentType: false,
        processData: false,
...
$(document).on("click",".take_action",function(){
  var id=$(this).attr('id')
   ...
   var status=$("#order").val()
     ...
     $.ajax({
        url:APP_URL+'/factory_order_status_update_by_account',
...
if ($.fn.DataTable.isDataTable('.yajra-datatable')) {
   $('.yajra-datatable').DataTable().destroy()
    ...
    var table = $('.yajra-datatable').DataTable({
        ...
        stateSave: $statesave,
        ajax: {
        url: "{{ route('get_factory_order_accounts') }}",
