...
<div class="row">
 <section class="col-lg-12 connectedSortable">
  ...
  ?>
   @if($request_segments=='Manage-Factory-Orders') 
 <input type="hidden" name="order" id="order" value="1">
    @elseif($request_segments=='Factory-Orders-Dispatched')
 <input type="hidden" name="order" id="order" value="2">
     @elseif($request_segments=='Factory-Orders-Delivered')
...
<script type="text/javascript">
 ...
 var APP_URL=$("#APP_URL").val();
  $.ajax({
        url:APP_URL+'/update_order_status_factory',
        data:form_data,
        type:'post',
        contentType: false,
        processData: false,
...
if ($.fn.DataTable.isDataTable('.yajra-datatable')) {
   $('.yajra-datatable').DataTable().destroy()
    ...
    var table = $('.yajra-datatable').DataTable({
        ...
        stateSave: $statesave,
        ajax: {
        url: "{{ route('get_factory_outlet_orders') }}",
