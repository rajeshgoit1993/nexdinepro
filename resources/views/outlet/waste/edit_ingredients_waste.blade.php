...
<script type="text/javascript" src="{{url('/resources/assets/pos')}}/frequent_changing/js/add_ingredients_waste.js"></script>
@endsection
@section('maincontent')
<style type="text/css">
   .ir_c_transparent {
...
<div class="row">
  ...
  <div class="flex-item-left"><h5>Edit Ingredients Waste</h5></div>
                            ...
                            <div class="form-group">
                                ...
                                <select  class="form-control select2  ir_w_100"
                                    ...
                                    <?php foreach ($ingredients as $ingredient) { 
                                        ...
                                        ?>
                                    <option 
                                        value="<?php echo POS_SettingHelpers::escape_output($ingredient->id . "|" . $ingredient->product_name . "(" . $ingredient->item_code . ")|" . CustomHelpers::get_master_table_data('units','id',$ingredient->unit,'unit')) ?>"
                                       > 
                                       <?php echo POS_SettingHelpers::escape_output($ingredient->product_name . "(" . $ingredient->item_code . ")") ?>
                                        </option>
                                    <?php } ?>
                                </select>
                            </div>
...
<tr class="rowCount" data-item_id="{{$ing->ingredient_id}}" data-id="{{$i}}" id="row_{{$i}}">
    ...
    <td><input type="text" required="" data-countid="{{$i}}"  id="quantity_amount_{{$i}}" name="quantity_amount[]" onfocus="this.select();" class="form-control integerchk aligning countID" placeholder="Qty" value="{{$ing->waste_amount}}"><span class="label_aligning">{{CustomHelpers::get_master_table_data('units','id',CustomHelpers::get_master_table_data('master_products','id',$ing->ingredient_id,'unit'),'unit')}}</span></td>

    <td><a class="btn btn-danger btn-xs" style="margin-left: 5px; margin-top: 10px;" onclick="return deleter({{$i}},{{$ing->ingredient_id}});"><i style="color:white" class="fa fa-trash"></i> </a></td>
