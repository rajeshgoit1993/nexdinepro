...
<table>
  <tr>
            <th colspan="{{$total_col}}" style="text-align: center;background: white">
                Combine Report
            </th>
...
$outlet_details=POS_SettingHelpers::get_outlet_details((int)$second_row);
  ...
  <td style="background:white;text-align: center;">{{ $outlet_details->firm_name }} ({{$outlet_details->city}})</td>
...
$payments = DB::table('franchise_sales')
                ->where([['outlet_id',$second_row],['del_status', 'Live'],['order_status',3],['payment_method_id',$payment_options->id]]) 
                ->whereIn('outlet_id',$ids_second) 
                ->whereDate('sale_date',$row)
                ->get(); 
...
$payments_other = DB::table('franchise_sales')
                ...
                ->where([['outlet_id',$second_row],['del_status', 'Live'],['order_status',3]]) 
                ->whereIn('outlet_id',$ids_second) 
                ->whereDate('sale_date',$row)
